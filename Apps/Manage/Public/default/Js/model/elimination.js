 ...
 $(function(){
    ...
    $(document).delegate('.ruleType', 'change', function(){
        if ($(this).val() == 3) {
            var wdateHtml = '<input class="Wdate" type="text" onClick="WdatePicker()" onfocus="WdatePicker(' + "{minDate:'#{%y-1}-%M-%d',maxDate:'#{%y+1}-%M-%d'}" + ')" name="er_value[]">';
			var wdateHtml_2 = '<input class="Wdate" type="text" onClick="WdatePicker()" onfocus="WdatePicker(' + "{minDate:'#{%y-1}-%M-%d',maxDate:'#{%y+1}-%M-%d'}" + ')" name="er_value_second[]">';
            ...
            var wdateHtml = '<input type="text" class="h30" name="er_value[]">';
			var wdateHtml_2 = '<input type="text" class="h30" name="er_value_second[]">';
            $(this).parent().parent().find('input[type=text]:eq(0)').replaceWith(wdateHtml);
			$(this).parent().parent().find('input[type=text]:eq(1)').replaceWith(wdateHtml_2);
        }
    });
