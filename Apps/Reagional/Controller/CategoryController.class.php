...
class CategoryController extends ReagionalController {
    public function shows() {
        $re_id = session('re_id');
        $type = strtolower(I('type'));
        if ($type == 'articleshows') {
            // 展示上级
            //$re_ids = explode('-', $re_id);
            //array_pop($re_ids);
            //$config['re_id'] = implode('-', $re_ids);
            $config['re_id'] = '';
            $config['s_id'] = 0;
        } else {
            // 展示自身
    ...
    public function addTpl($title = '', $result = array()) {
        ...
        // 默认值
        $result['re_id'] = session('re_id');
        $result['re_title'] = session('re_title');
        ...
        $addList[] = array('title' => '', 'name' => 're_id', 'type' => 'hidden');
        $addList[] = array('title' => '', 'name' => 're_title', 'type' => 'hidden');
