...
class ClassController extends ReagionalController {
    public function index() {
        // 显示字段
        $tplList['id'] = array('id' => 'c_id', 'percent' => '5', 'title' => L('NUMBER'));
        $tplList[] = array('id' => 'c_title', 'percent' => '15', 'title' => L('TITLE'), 'class' => 'showContent');
        $tplList[] = array('id' => 's_id', 'percent' => '0', 'title' => L('S_ID'));
        $tplList[] = array('id' => 's_title', 'percent' => '15', 'title' => L('S_TITLE'), 'class' => 'showContent');
        $tplList[] = array('id' => 're_title', 'percent' => '15', 'title' => L('REGION'), 'class' => 'showContent');
        ...
        $search[] = array('title' => L('S_TITLE'), 'name' => 's_title', 'inline' => true);
    ...
    public function addTpl($title = '', $result = array()) {
        ...
        // 隐藏项
        $tag = reloadCache('tag');
        $tag[4][0] = L('PLEASE_SELECT_TYPE');
        ksort($tag[4]);
        $school_divide = explode(',', C('SCHOOL_DIVIDE'));
        $school_divide[0] = L('PLEASE_SELECT_TYPE');
        $addList[] = array('name' => 's_type', 'title' => L('SCHOOL_TYPE'), 'label' => 'select', 'data' => $tag[4], 'class' => 'hide');
        $addList[] = array('name' => 's_divide', 'title' => L('SCHOOL_DIVIDE'), 'label' => 'select', 'data' => $school_divide, 'class' => 'hide');
    ...
    public function shows() {
        if ($_POST) {
            ...
            // 区域
            $re_id = $this->getSubjectRegion('-', session('re_id'), $gsr_list);
            // 学科
