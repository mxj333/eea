 ...
 $(document).ready(function(){
    ...
    $('.searchCondition input').live('change', function(){
        ...
        var s_id = $('.searchCondition input[name=s_id]').val();
        if (!account && !name) {
            // 重置
        } else {
            $.ajax({
 ...
 function authorization(c_id) {
    ...
    var school_id = $('input[type=checkbox][value='+c_id+']').parent().find('.s_id').text();
    if (!adviser_name) {
        adviser_name = '暂无';
    }
    var preHtml = searchResultHtml(c_id, '班主任设置', class_name + '  班主任：' + adviser_name, school_id);
 ...
 function searchConditionHtml(type) {
     return '<span><label>账号：</label><input type="text" name="account"></span><span><label>姓名：</label><input type="text" name="name"></span><input type="hidden" name="s_id" value="'+type+'">';
 }
