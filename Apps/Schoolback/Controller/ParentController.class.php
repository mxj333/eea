...
class ParentController extends SchoolbackController {
    public function index() {
        ...
        $action[] = array('id' => 'resume', 'percent' => '4', 'title' => L('CHILDREN'));

        // 检索器
        $search[] = array('title' => L('NICKNAME'), 'name' => 'me_nickname');
        $search[] = array('title' => L('MOBILE'), 'name' => 'me_mobile', 'inline' => true);
    ...
    public function addTpl($title = '', $result = array()) {
        ...
        $addList[] = array('name' => 'me_validity', 'title' => L('VALIDITY'), 'class' => 'w460', 'event' => 'onClick', 'labelClass' => 'Wdate', 'eventValue' => "WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#{%y}-{%M}-#{%d+1}'});", 'require' => true);
    ...
    public function shows() {
        ...
        $this->assign('archives_type', array(8 => $archives_type[8]));
    ...
    public function user() {
        ...
        // 教学能力
        if (intval($_POST['mar_type']) ==8 && $result['list']) {
            // 只能添加一个
        } else {
    ...
    public function resume() {
        ...
        $this->everyModelCss = intval(file_exists('.' . MPUBLIC_NAME . '/' . C('DEFAULT_THEME') . '/Css/studentResume.css'));

        $config['parent_id'] = I('id');
        $config['is_deal_result'] = true;
        ...
        $this->assign('parent_id', I('id', 0, 'intval'));
        $this->display();
    }
}