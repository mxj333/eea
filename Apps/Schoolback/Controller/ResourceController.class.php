...
class ResourceController extends SchoolbackController {
    ...
    public function edit() {
        ...
        if ($_POST) {
            ...
            // 获取数据
            $vo = $this->apiReturnDeal(getApi(array('res_id' => intval(I('request.id')), 'belong' => 3, 'is_deal_result' => true), 'Resource', 'shows'));
            foreach ($vo as &$value) {
    ...
    public function insert() {
        if (intval($_POST['res_id']) && !is_array($_POST['res_id'])) {
            ...
            // 处理发布位置
            $published_local = ($_POST['res_is_published'] == 1) ? intval($_POST['published_local']+4) : intval($_POST['published_local']);

            unset($saveData);
            $saveData = D('Resource')->localToFields($published_local, array('re_id' => session('s_re_id'), 're_title' => session('s_re_title')));
            // 学校后台学校标识
            $saveData['s_id'] = session('s_id');
            ...
            foreach($_POST['res_title'] as $key => $val) {
                ...
                // 处理发布位置
                $published_local = ($data['res_is_published'] == 1) ? intval($_POST['published_local'][$key]+4) : 0;

                unset($saveData2);
                $saveData2 = D('Resource')->localToFields($published_local, array('re_id' => session('s_re_id'), 're_title' => session('s_re_title')));
                // 学校后台标识
                $saveData2['s_id'] = session('s_id');
