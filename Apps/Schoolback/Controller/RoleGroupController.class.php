...
class RoleGroupController extends SchoolbackController {
    ...
    public function user() {
        ...
        if ($_POST) {
            ...
            $config['rg_id'] = intval($_POST['rg_id']);
            $config['auth_id'] = strval($_POST['auth_id']);
            // 请求接口
            $res = $this->apiReturnDeal(getApi($config, 'RoleGroup', 'user'));
            // 返回值
            ...
            // 获取角色用户
            $member_list = $this->apiReturnDeal(getApi(array('rg_id' => intval(I('id')), 'is_deal_result' => false), 'RoleGroup', 'getUser'));
            $this->assign('member_list', $member_list);

            // 所有管理员
            $config['s_id'] = $_SESSION['s_id']; // 用户所属区域id
            $config['is_deal_result'] = true; // 结果自动处理
            $config['belong'] = 3; // 学校
            $config['is_page'] = false;
            $managers = $this->apiReturnDeal(getApi($config, 'SchoolManager', 'lists'));
            $this->assign('managers', $managers['list']);
    ...
    public function index() {
        ...
        $tplList[] = array('id' => 'rg_title', 'percent' => '45', 'title' => L('NAME'), 'class' => 'showContent');
