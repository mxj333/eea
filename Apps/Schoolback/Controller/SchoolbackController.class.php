...
class SchoolbackController extends Controller {
    public function _initialize() {
        ...
        cacheData();
        C('DEFAULT_THEME', C('SCHOOLBACK_DEFAULT_THEME'));
    ...
    public function add($html = '') {
        if ($_POST) {
            ...
            if ($html) {
                ...
                $vo['re_id'] = session('s_re_id');
                $vo['re_title'] = session('s_re_title');
                $this->assign('vo', $vo);
    ...
    public function delete($pk, $table = '') {
        ...
        // api 请求数据
        $data = $this->apiReturnDeal(getApi(array($pk => I('id', 0, 'strval'), 'belong' => '3'), $table, 'del'));
